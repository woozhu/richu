...
import(
    "math"
    "github.com/woozhu/wxTime/astronomy"
    "github.com/woozhu/wxTime/orbital"
)
...
func geodetic_lat(point POINT,a,b float64) float64{
    var(
        x,y,z=point.x,point.y,point.z
        r=math.sqrt(x*x+y*y)
        geoc_lat = math.arctan2(z,r)
        geod_lat = geoc_lat
        e2=(a*a-b*b)/(a*a)
    )
    for true{
        ...
        geod_lat = math.arctan2(z+a*C*e2*sin(phi),r)
        if math.allclose(geod_lat,phi){
            break
        }
    }
    return geod_lat
