...
// Package `WXTime` is an extension to Go's time library based on PHP's WXTime library
package WXTime
...
// The WXTime type represents a Time instance.
...
// 1 hour after
// 5 months after
func (c *WXTime) DiffForHumans(d *WXTime, abs, absolute, short bool) (string, error) {
	isNow := (d == nil)
	if isNow {
		d = Now()
	}
...
// GetTranslator returns Translator inside a WXTime instance if exist,
// otherwise it creates a new Translator with "en" as default locale
func (c *WXTime) GetTranslator() (*Translator, error) {
	if c.Translator == nil {
...
// SetTranslator sets the Translator inside a WXTime instance
func (c *WXTime) SetTranslator(t *Translator) {
	c.Translator = t
}

// GetLocale returns locale of the Translator of WXTime
func (c *WXTime) GetLocale() string {
	return c.Translator.GetLocale()
}

// SetLocale sets locale for the Translator of WXTime
func (c *WXTime) SetLocale(l string) error {
...
// FormatLocalized will format the WXTime instance with the current locale
func (c *WXTime) FormatLocalized(format string) (string, error) {
